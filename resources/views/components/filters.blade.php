...
<form method="GET" action="{{ $action }}" class="flex flex-col gap-3 mb-6">
    ...
    @if($search)
        <div class="flex gap-3">
            ...
            <button type="submit" class="btn btn-primary px-8 font-normal">Поиск</button>
        </div>
    @endif
    ...
    @if($slot->isNotEmpty())
        <div class="flex flex-wrap gap-3">
            {{ $slot }}
            <button type="submit" class="btn btn-primary font-normal">Применить</button>
            @if(request()->hasAny(array_keys(request()->except('page'))))
                <a href="{{ $action }}" class="btn font-normal">Сбросить</a>
            @endif
        </div>
    @endif
